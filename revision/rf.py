...
def evaluate_fold(rest_Mv, rest_meta, holdout_Mv, holdout_meta):
    """Train and evaluate multilabel RandomForest"""
...
def main():
    ...
    # Save results
    timestamped_print("\nSaving results...")
    with open(f"{REVISION_RESULTS_PATH}/rf_results.dill", "wb") as f:
        dill.dump(fold_results, f)
    with open(f"{REVISION_RESULTS_PATH}/rf_clfs.dill", "wb") as f:
        dill.dump(fold_clfs, f)

    # Export also to CSV for easy inspection
    pd.DataFrame(fold_results).T.to_csv(f"{REVISION_RESULTS_PATH}/rf_results.csv")
